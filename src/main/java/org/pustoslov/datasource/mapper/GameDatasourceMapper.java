...
public class GameDatasourceMapper {
  ...
  public GameEntity toDataEntity(Game game) {
    ...
    GameEntity entity;
    if (game.getId() != null) {
      entity = new GameEntity(game.getId(), boardList, game.getStatus(), game.getCurrentTurn(),
              game.getWinner(), game.getXPlayerId(), game.getOPlayerId());
    } else  {
  ...
  public Game toDomain(GameEntity entity) {
    int[][] matrix = entity.getBoard().stream()
            .map(row -> row.stream().mapToInt(Integer::intValue).toArray())
            .toArray(int[][]::new);

    return new Game(entity.getId(), new Board(matrix), entity.getStatus(), entity.getCurrentTurn(),
            entity.getWinner(), entity.getxPlayerId(), entity.getoPlayerId());
  }
}
